...
import {PumpkitServiceManager} from "../../src/PumpkitServiceManager.sol";
...
library PumpkitDeploymentLib {
    ...
    function deployContracts(
        ...
        result.pumpkitServiceManager = UpgradeableProxyLib.setUpEmptyProxy(proxyAdmin);
        result.stakeRegistry = UpgradeableProxyLib.setUpEmptyProxy(proxyAdmin);
        // Deploy the implementation contracts, using the proxy contracts as inputs
        address stakeRegistryImpl =
            address(new ECDSAStakeRegistry(IDelegationManager(core.delegationManager)));
        address pumpkitServiceManagerImpl = address(
            new PumpkitServiceManager(
                core.avsDirectory, result.stakeRegistry, core.rewardsCoordinator, core.delegationManager
            )
        );
        // Upgrade contracts
        bytes memory upgradeCall = abi.encodeCall(
            ECDSAStakeRegistry.initialize, (result.pumpkitServiceManager, 0, quorum)
        );
        UpgradeableProxyLib.upgradeAndCall(result.stakeRegistry, stakeRegistryImpl, upgradeCall);
        UpgradeableProxyLib.upgrade(result.pumpkitServiceManager, pumpkitServiceManagerImpl);
    ...
    function readDeploymentJson(
        ...
        data.pumpkitServiceManager = json.readAddress(".contracts.pumpkitServiceManager");
    ...
    function writeDeploymentJson(
        ...
        address proxyAdmin =
            address(UpgradeableProxyLib.getProxyAdmin(data.pumpkitServiceManager));
    ...
    function _generateContractsJson(
        ...
        return string.concat(
            ...
            '","pumpkitServiceManager":"',
            data.pumpkitServiceManager.toHexString(),
            '","pumpkitServiceManagerImpl":"',
            data.pumpkitServiceManager.getImplementation().toHexString(),
            '","stakeRegistry":"',
